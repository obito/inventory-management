...
def create_restocking_order(request: RestockingOrderRequest):
    ...
    if not request.items:
        raise HTTPException(status_code=422, detail="Restocking order must include at least one item")

    # Lead time is the slowest line item, since the order can't arrive until every item does
    lead_times_by_sku = {f["item_sku"]: f["lead_time_days"] for f in demand_forecasts}
    lead_time_days = max(
        (lead_times_by_sku.get(item.item_sku, 14) for item in request.items),
        default=14
    )
...
def get_category_spending():
    """Get spending by category with each category's share of total spend"""
    # The `percentage` field in spending.json is stale (the stored values summed to
    # 123.6% and did not track the amounts), so derive it from the amounts instead.
    total = sum(category.get('amount', 0) for category in category_spending)

    result = []
    for category in category_spending:
        entry = dict(category)
        entry['percentage'] = round((entry.get('amount', 0) / total) * 100, 1) if total else 0.0
        result.append(entry)
...
def create_purchase_order(request: CreatePurchaseOrderRequest):
    ...
    if any(po["backlog_item_id"] == request.backlog_item_id for po in purchase_orders):
        raise HTTPException(
            status_code=400,
            detail=f"A purchase order already exists for backlog item {request.backlog_item_id}"
        )
...
def get_purchase_order_by_backlog_item(backlog_item_id: str):
    """Get the purchase order raised against a specific backlog item"""
    purchase_order = next(
